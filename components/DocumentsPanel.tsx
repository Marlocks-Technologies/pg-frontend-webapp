...
/**
 * Read any file as a standard base64 string (no data-URL prefix).
 * The server expects raw base64 — it handles all parsing/extraction on its end.
 */
function readAsBase64(file: File): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = e => {
      ...
      // dataUrl = "data:<mime>;base64,<base64data>" — strip the prefix
      const base64 = dataUrl.split(',')[1];
      if (!base64) reject(new Error('Could not encode file as base64'));
      else resolve(base64);
    };
    reader.onerror = () => reject(new Error('Could not read file'));
...
function UploadModal({
  ...
  // Process any picked file → encode as base64 for the API
  const processFile = async (file: File) => {
    setPickedFile(file);
    setFilename(file.name);
    setError('');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          >
            ...
            {extracting ? (
              <>
                ...
                <p className={`text-[12.5px] font-medium ${isDark ? 'text-white' : 'text-charcoal/55'}`}>
                  Extracting text…
                </p>
              </>
            ) : pickedFile ? (
              <>
                ...
                <p className={`text-[11px] ${isDark ? 'text-white' : 'text-charcoal/35'}`}>
                  {formatBytes(pickedFile.size)} · click to change
                </p>
              </>
            ) : (
          ...
          {/* Content — shows base64 status when file is picked, manual paste otherwise */}
          {pickedFile ? (
            <div className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl border
              ${isDark ? 'bg-emerald-400/6 border-emerald-400/20' : 'bg-emerald-50 border-emerald-200'}`}
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"
                className={isDark ? 'text-emerald-400 shrink-0' : 'text-emerald-600 shrink-0'}
              >
                <polyline points="20,6 9,17 4,12"/>
              </svg>
              <div>
                <p className={`text-[12.5px] font-medium ${isDark ? 'text-emerald-400' : 'text-emerald-700'}`}>
                  File encoded — ready to upload
                </p>
                <p className={`text-[11px] mt-0.5 ${isDark ? 'text-emerald-400/60' : 'text-emerald-600/70'}`}>
                  {content.length.toLocaleString()} base64 chars · server will extract text
                </p>
              </div>
            </div>
          ) : (
            <Field label="Content *" isDark={isDark}>
              <textarea
                value={content}
                onChange={e => setContent(e.target.value)}
                placeholder="Or paste plain text content here directly…"
                rows={5}
                className={`${inputCls(isDark)} resize-none`}
              />
            </Field>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4">
        {loading ? (
          ...
          <div className={`flex flex-col items-center justify-center h-full gap-3 ${isDark ? 'text-white/18' : 'text-charcoal/22'}`}>
            ...
            <div className="text-center">
              <p className="text-[13px] font-medium">{search ? 'No matches found' : 'No documents yet'}</p>
              {!search && <p className="text-[11px] mt-0.5 opacity-60">Upload your first document to get started</p>}
            </div>
          </div>
        ) : (
          <div className="space-y-2">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    ...
                    {doc.size !== undefined && (
                      <span className={`text-[10px] ${isDark ? 'text-white' : 'text-charcoal/28'}`}>
                        {formatBytes(doc.size)}
                      </span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-1 opacity-100 transition-opacity duration-150 sm:opacity-0 sm:group-hover:opacity-100">
                  ...
                  <button
                    ...
                    title="View details"
                    className={`p-2 rounded-lg transition-colors
                      ${isDark ? 'text-white hover:text-white hover:bg-white/8' : 'text-charcoal/30 hover:text-charcoal/70 hover:bg-charcoal/6'}`}
                  >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <circle cx="12" cy="12" r="10"/>
                      <line x1="12" y1="8" x2="12" y2="12"/>
                  ...
                  <button
                    ...
                    className={`p-2 rounded-lg transition-colors
                      ${isDark
                        ? 'text-white hover:text-red-400 hover:bg-red-400/10 disabled:opacity-30'
                        : 'text-charcoal/25 hover:text-red-500 hover:bg-red-50 disabled:opacity-30'
                      }`}
                  >
                    {deletingId === doc.documentId ? (
